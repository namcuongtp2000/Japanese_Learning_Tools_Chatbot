...
def translate_from_ja_to_en(patterns):
    ...
    print('Translatation: '+result_dest)
    return result_dest
...
def putJapaneseText(img, x, y, text, text_en, size=32):
    if os.name =='nt':
    ...
    try:
        ...
        draw.text((x, y-h*1.2), '%s - %s'%(text,text_en), font=font, fill=(255,0,0,0))
    ...
    while True:
        ...
        for i, rect in enumerate(rects):
            ...
            if result != "['']":
                ...
                idx1 = str(result).index(sub1)
                idx2 = str(result).index(sub2)

                result2s = ''
            
                # getting elements in between
                for idx in range(idx1 + len(sub1) , idx2):
                    result2s = result2s + str(result)[idx]
                print(result2s)
            else:
                pass
            canvas2 = putJapaneseText(canvas2, most_left[0], most_left[1], result[0] , translate_from_ja_to_en(result2s))
            
            #speak_ja(str(result2s))
        ...
        print(if_kanji(result2s))
        #Speak
        mixer.init()
        sound = speak_ja(result2s)
        sound.seek(0)
